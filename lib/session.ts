...
export type SessionConfig = { sessionId: string; participant: string; sessionLabel: string; category?: string; sign: string; take: string; takeId: string; createdAt: number };
export type CameraStatus = { view: CameraView; deviceId: string; takeId?: string; ready: boolean; state: 'connected' | 'ready' | 'countdown' | 'recording' | 'uploading' | 'complete' | 'error'; updatedAt: number; videoUrl?: string; landmarksUrl?: string; filename?: string; error?: string };
export type CaptureCommand = { id: string; type: 'idle' | 'start' | 'stop'; issuedAt: number; startAt?: number; stopAt?: number; takeId: string };
...
export function captureStem(config: SessionConfig, view: CameraView) { return [config.sign, config.take, view].map((v) => safeToken(v, 'unknown')).join('_'); }
